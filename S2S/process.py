...
class Forecast:
    ...
    Loads forecasts from S2S database, computes weekly means and then provides
        ...
        self.data_a:    the anomlies of the data relative to model hindcast climatology
                        (xarray.DataArray)
        self.mean:      the model hindcast mean climatology (over 30-day running window)
                        (xarray.DataArray)
        self.std:       the model hindcast std climatology (over 30-day running window)
                        (xarray.DataArray)
    ...
    def load_data(self):
        ...
        return ECMWF_S2SF(high_res=self.high_res)\
                        .load(
                                self.var,
                                self.t_start,
                                self.t_end,
...
class Observations:
    ...
    def __init__(
        ...
        if self.process or not os.path.exists(self.path+filename_absolute):
            ...
            # what is the renaming for and why does it fail (for me)?
